...
$productsPopular = mysqli_query($ConnectDatabase, "SELECT * FROM `products_profitable`");
$productsPopular = mysqli_fetch_all($productsPopular, MYSQLI_ASSOC);


$countPopular = count($productsPopular);

if ($countPopular > 4) {
    $countPopular = $countPopular - 3;
} else {
    $countPopular = 1;
}

function addSliderUser($productsAll, $photos, $productsPopular)
{
...
function addSliderAdm($productsAll, $photos, $productsPopular)
{
